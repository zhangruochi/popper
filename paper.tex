...
Structure-based binder design has advanced significantly through methods such as RFDiffusion~\cite{watson2023novo} combined with ProteinMPNN~\cite{dauparas2022robust}, which leverage geometric and physical constraints to generate protein--peptide complexes. These approaches excel at producing structurally plausible candidates by incorporating folding constraints and interface geometry directly into the generation process~\cite{alford2017rosetta}. Structure prediction methods for protein-ligand complexes~\cite{bryant2024structure} and interface prediction tools~\cite{krapp2023pesto} provide complementary capabilities for assessing binding interfaces. Sequence-based peptide language models and masked language model (MLM) mutation sampling, such as~\cite{chen2025target}, provide alternative strategies that operate on sequence grammars and in silico evolution principles, enabling efficient exploration of sequence space while respecting biochemical constraints. Recent advances in protein language models~\cite{ferruz2022controllable,DBLP:conf/icml/AdamsBLYA25,DBLP:conf/nips/WangDQLZZ0ZC24} have demonstrated the power of large-scale sequence representations for structure and function prediction.
...
Multi-objective evolutionary algorithms, particularly NSGA-II~\cite{ma2023comprehensive} and related Pareto-based optimizers~\cite{zhao2023hypervolume}, provide well-established frameworks for handling conflicting objectives. Flow-based approaches for multi-objective optimization~\cite{DBLP:conf/iclr/00170P025} and methods for learning latent Pareto sets~\cite{liu2025multi} offer alternative strategies for exploring trade-off surfaces. These methods maintain diverse solution sets through non-dominated sorting and crowding distance metrics, enabling exploration of trade-off surfaces without premature convergence to suboptimal regions. Bayesian optimization~\cite{wang2023recent,sabanza2025best} and bandit-style methods offer complementary approaches for efficient exploration under limited feedback, using probabilistic models to guide sample selection and balance exploration with exploitation~\cite{slautin2025measurements}.
...
Recent advances in tool-augmented LLM agents~\cite{naveed2025comprehensive} have demonstrated the potential for combining language model reasoning with specialized computational tools. In scientific domains, these agents can orchestrate complex workflows involving multiple tools, dynamically routing tasks and integrating results~\cite{wang2024agent}. Agent frameworks for modular design~\cite{DBLP:conf/iclr/ShangLZMLXL25}, decision-making through Q-learning~\cite{DBLP:conf/iclr/ChaiLFZZ25}, evaluation and benchmarking~\cite{mohammadi2025evaluation}, and security considerations~\cite{he2025emerged} have expanded the capabilities and understanding of agentic systems. Applications in medicine~\cite{thirunavukarasu2023large} further demonstrate the practical impact of LLM agents. However, a critical distinction exists between bounded loops with structured state and open-ended chat agents that lack explicit termination conditions and reproducible execution paths.
...
\end{equation}
where $w_{\text{conf}} + w_{\text{geom}} + w_{\text{energ}} = 1$ (default: 0.30, 0.40, 0.30). The \textbf{interface confidence} component $u_{\text{conf}}(x) = \text{iPTM}(x)$ is the interface predicted TM-score from structure prediction~\cite{jumper2021highly,evans2021protein}, directly measuring interface region reliability. The \textbf{interface geometry} component $u_{\text{geom}}(x) = \frac{1}{2}(\text{interface\_sc}(x) + \text{interface\_packstat}(x))$ averages interface score and packing statistics, both normalized to $[0,1]$. The \textbf{interface energetics} component $u_{\text{energ}}(x) = \frac{1}{3}\left(\tilde{u}_{\text{binder}}(x) + \tilde{u}_{\Delta G}(x) + \tilde{u}_{\text{hbonds}}(x)\right)$ averages three normalized energy metrics:
\begin{align}
...
\end{equation}
where $w_{\text{interface}} = 0.60$ (default) and $u_{\text{backbone}}(x) = \text{normalize}(\text{complex\_pLDDT}(x))$ is the normalized backbone confidence score from structure prediction~\cite{jumper2021highly,passaro2025boltz}.
...
\end{equation}
where $|\Delta(p, x)|$ is the number of mutated positions. Constraint violations are detected algorithmically: protected position mutations are rejected immediately, mutation budget violations are filtered, and vocabulary violations are corrected or rejected. This pre-filtering ensures that invalid candidates do not consume evaluation budget (structure prediction~\cite{jumper2021highly,passaro2025boltz}, energy scoring), which is often the computational bottleneck.
...
\textbf{Scenario B: Rich SAR (Mid-Stage Optimization).} This regime reflects projects with more mature SAR data ($>30$ entries), typically obtained from literature mining. We focus on well-characterized protein--peptide interaction benchmarks, including PDZ, Bcl-2, GLP-1, MDM2, and specifically the PD-L1/PD-1 interface as the primary optimization target. In this scenario, we utilize the full evaluation pipeline, including structure prediction (Boltz-2~\cite{passaro2025boltz}) and binding energetics assessment, to provide high-fidelity feedback.
...
We compare against four baselines spanning structure-based, sequence-based, heuristic, and generalist paradigms. The structure-based baseline uses RFDiffusion combined with ProteinMPNN (RFD+MPNN)~\cite{watson2023novo,dauparas2022robust} to generate designs guided primarily by geometry and inverse folding~\cite{sumida2024improving}. The sequence-based baseline uses PepMLM-style masked language modeling (PepMLM)~\cite{chen2025target} to propose statistically plausible substitutions via token sampling conditioned on the parent sequence. The heuristic baseline uses NSGA-II~\cite{ma2023comprehensive} on the exact same scoring oracle as our method, providing a multi-objective optimizer without explicit reasoning. The direct LLM baselines use single-pass Qwen3-32B~\cite{yang2025qwen3} and DeepSeek-V3.2~\cite{deepseekai2024deepseekv3technicalreport} prompts without tool access or multi-round updates, testing whether iterative optimization provides benefits over one-shot generation under weak feedback.
...
Constraint satisfaction is evaluated through charge bounds, aggregation risk assessments, structural thresholds including pLDDTcarugo2023plddt~\cite{carugo2023plddt} and iPTM~\cite{jumper2021highly} cutoffs, and protected-position violation detection. These metrics ensure that optimization does not sacrifice feasibility for score improvements.

Pareto-front quality is assessed through first-front size (number of non-dominated solutions), hypervolume~\cite{zhao2023hypervolume,deutz2023hypervolume} in normalized objective space (measuring the volume dominated by the solution set), and diversity statistics computed from crowding distances. These metrics capture both the quality and diversity of the solution set, enabling assessment of trade-off exploration.
...
\caption{Pareto trade-offs and convergence (Scenario B / PD-L1). The dashboard reports the candidate cloud, Pareto-front projection, convergence across rounds, hypervolume~\cite{zhao2023hypervolume,deutz2023hypervolume}, and rank distribution, providing an information-dense view of multi-objective optimization under structured feedback.}
...
Table~\ref{tab:structure_validity} reports structural validity (pLDDT, iPTM~\cite{jumper2021highly}) and energetic assessment (Interface $\Delta G$) from independent Boltz-2 structure prediction~\cite{passaro2025boltz}. As expected, RFD+MPNN achieves the highest pLDDT and iPTM scores because it natively generates valid protein geometries through diffusion-based backbone generation. Note that pLDDT values in AlphaFold2 models have been shown to be unrelated to local flexibility~\cite{carugo2023plddt}, highlighting the importance of complementary metrics. However, our method achieves comparable structural confidence (pLDDT: 0.830 vs 0.885, iPTM: 0.781 vs 0.835) while offering significantly better binding energetics (lowest $\Delta G$: -12.516 vs -11.855), demonstrating that the Pareto-based optimization successfully balances structural plausibility with binding potency. Structure prediction methods~\cite{evans2021protein} continue to improve, but integration with optimization loops remains challenging.
